...
<html>
<head>
    <title>SWISA-AGAP Grant Request Form</title>
    {{-- Use inline CSS for guaranteed styling in Dompdf --}}
    <style>
        ...
        .section-header { 
            font-size: 12pt; font-weight: bold; color: #2C6E49; border-bottom: 2px solid #ccc; 
            padding-bottom: 3px; margin-top: 20px; margin-bottom: 10px;
        }
        .data-table { width: 100%; border-collapse: collapse; }
        .data-table td { padding: 5px 0; vertical-align: top; width: 50%; }
        .data-label { font-weight: bold; color: #555; display: inline-block; width: 140px; }
        .data-value { 
        ...
        .declaration-box { border: 1px solid #ddd; padding: 10px; margin-top: 30px; font-size: 9pt; background-color: #f9f9f9; }
        .signature-line { border-top: 1px solid #000; width: 200px; margin-top: 40px; text-align: center; font-size: 9pt; }
    </style>
</head>
<body>
    ...
    <div class="container">
        ...
        <table style="width: 100%; margin-bottom: 15px;">
            <tr>
                <td style="width: 70%;">
                    <h1 class="app-title">SWISA-AGAP Grant Request Application</h1>
                    <p class="dept-info">Department of Agriculture, Small Water Irrigation System Association Federation</p>
                    <p style="font-size: 9pt; color: #888; margin-top: 5px;">Application ID: {{ $application->id ?? 'N/A' }} | Date: {{ now()->format('F d, Y') }}</p>
                </td>
        ...
        <div class="section-header">1. REQUEST DETAILS</div>
        <table class="data-table">
            <tr>
                <td colspan="2"><span class="data-label">Grant Title:</span><span class="data-value" style="min-width: 600px;">
                    {{ $application->grant->title ?? 'N/A' }}
                </span></td>
            </tr>
            <tr class="full-width">
                <td colspan="2"><span class="data-label">Purpose of Request:</span><span class="data-value" style="min-width: 600px;">
                    {{ $application->purpose ?? 'N/A' }}
                </span></td>
            </tr>
            <tr>
                <td><span class="data-label">Application Status:</span><span class="data-value">
                    {{ ucfirst($application->status->status_name ?? 'Pending') }}
                </span></td>
            </tr>
        </table>

        <div class="section-header">2. APPLICANT PERSONAL INFORMATION</div>
        <table class="data-table">
            <tr>
                <td><span class="data-label">Full Name:</span><span class="data-value">
                    {{ $application->user->first_name ?? '' }} {{ $application->user->middle_name ?? '' }} {{ $application->user->last_name ?? '' }} {{ $application->user->suffix ?? '' }}
                </span></td>
                <td><span class="data-label">Contact No.:</span><span class="data-value">
                    {{ $application->user->user_info->phone_no ?? '-' }}
                </span></td>
            </tr>
            <tr>
        ...
        <div class="section-header">3. SECONDARY CONTACT</div>
        <table class="data-table">
            <tr>
        ...
        <div class="section-header">4. AGRICULTURE / LIVELIHOOD INFORMATION</div>
        <table class="data-table">
            <tr>
        ...
        <div class="section-header">5. REQUIREMENTS ATTACHED</div>
        <div style="padding-left: 10px;">
            @forelse($application->documents as $doc)
                <p style="margin-bottom: 2px;">
                    <span style="font-weight: bold; color: #4C956C;">&#10003;</span> 
                    {{ $doc->grantRequirement->requirement->requirement_name ?? 'Document Attached' }}
                </p>
            @empty
        ...
        <div class="declaration-box">
            ...
            <p style="margin-bottom: 10px;">I hereby certify that the information I provided above is true and correct to the best of my knowledge. I understand that any false statement or misrepresentation shall be grounds for disqualification or termination of any grant or assistance from SWISA-AGAP.</p>
        </div>
        ...
        <table style="width: 100%; margin-top: 50px;">
            <tr>
                ...
                <td style="width: 50%; text-align: right;">
                    <p style="font-size: 9pt; color: #888; margin-top: 40px;">For Official Use Only</p>
                </td>
            </tr>
        </table>
