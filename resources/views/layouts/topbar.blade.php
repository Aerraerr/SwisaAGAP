...
<div class="topbar">
  ...
  <div id="user-widget" class="flex justify-between items-center gap-4 px-3 py-1 bg-white rounded-lg transition-all duration-300 ease-in-out relative">
    ...
    <button id="notifButton" class="relative w-9 h-9 flex items-center justify-center rounded-full hover:bg-gray-100 transition">
      ...
      <span class="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] px-1.5 rounded-full">{{ $notifications->count() }}</span>
    </button>
...
<div id="notifPanel" class="fixed top-12 mt-3 right-0 h-full w-[500px] max-w-[90%] bg-white shadow-2xl z-10 transform translate-x-full transition-transform duration-300 ease-in-out">
  ...
  <div class="p-4 overflow-y-auto h-[calc(100%-3rem)]">
    <p class="text-gray-600 text-sm">🔔 You have {{ $notifications->count()}} notifications.</p>
    <div class="mt-3 space-y-2">
    
      <!-- Today notifications -->
      @if($todayNotifications->count() > 0)
        <h3 class="mt-4 mb-2 font-semibold text-gray-700 text-sm uppercase">Today</h3>
        @foreach($todayNotifications as $notif)
          <div class="p-2 bg-green-50 rounded border border-green-100 text-gray-800">
            <p class="text-sm">{{ $notif->message }}</p>
            <p class="text-xs text-customIT font-bold mt-1">{{ $notif->created_at->diffForHumans() }}</p>
        </div>
        @endforeach
      @endif

      <!-- Past notifications -->
      @if ($pastNotifications->count() > 0)
        <h3 class="mt-4 mb-2 font-semibold text-gray-700 text-sm uppercase">Earlier</h3>
        @foreach ($pastNotifications as $notif)
          <div class="p-2 bg-gray-50 rounded border border-gray-100 text-gray-700">
            <p class="text-sm">{{ $notif->message }}</p>
            <p class="text-xs text-customIT font-bold mt-1">{{ $notif->created_at->format('M d, Y h:i A') }}</p>
          </div>
        @endforeach
      @endif
      ...
      @if ($notifications->count() === 0)
        <div class="p-2 rounded text-center text-gray-500">No notifications...</div>
      @endif
    </div>
  </div>
</div>
