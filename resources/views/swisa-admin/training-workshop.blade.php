...
<div class="p-4 -mt-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl shadow p-4 border-t-4 border-blue-600">
            ...
            <p class="text-3xl font-bold text-blue-600 mt-2">{{ $upcomingEvents}}</p>
        ...
        <div class="bg-white rounded-2xl shadow p-4 border-t-4 border-yellow-500">
            ...
            <p class="text-3xl font-bold text-yellow-600 mt-2">{{ $completedEvents}}</p>
    ...
    <div class="bg-white  p-5 rounded-xl shadow-xl border border-gray-300">
        <div x-data="{ activeTab: 'grid' }" class="mt-4">
            ...
            <div x-show="activeTab === 'grid'" id="training-card-container" class="pt-2  grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                ...
                @foreach($trainings as $training)
                    <x-cards.training-card
                        ...
                        data-search="{{ strtolower($training->title.' '.$training->sector->sector_name.' '.$training->venue.' '.$training->date->format('F d Y').' '.\Carbon\Carbon::createFromFormat('H:i:s', $training->time)->format('g:i A')
                        .' '.($training->date < now() ? 'Completed' : 'Upcoming')) }}"
                    />
                @endforeach
            </div>
