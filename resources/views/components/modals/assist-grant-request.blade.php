<div id="assistGrantRequestModal-{{ $member->id}}" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 z-20 h-full w-full flex items-center justify-center overflow-y-auto">
  <div class="relative w-auto max-w-6xl mx-auto p-6 border shadow-lg rounded-xl bg-white transition-transform transform scale-95 duration-300">
    ...
    @csrf
        ...
        <fieldset class="border border-gray-300 p-4 mt-4 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="email" name="email" value="{{ $member->email ?? 'email'}}" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" class="peer invalid:[&:not(:placeholder-shown):not(:focus)]:border-red-500 w-full mt-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-btncolor focus:border-btncolor focus:ring-opacity-30" required>
                <span class="mt-2 hidden text-sm text-red-500 peer-[&:not(:placeholder-shown):not(:focus):invalid]:block">
        ...
        <fieldset class="border border-gray-300 p-4 mt-4 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="text" name="sc_purok" value="{{ $member->user_info->sc_purok ?? 'purok'}}" class="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-btncolor focus:border-btncolor focus:ring-opacity-30" required>
              </div>
              <div>
        ...
        <fieldset class="border border-gray-300 p-4 mt-4 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="text" name="farm_location" value="{{ $member->user_info->farm_location ?? 'farm location'}}" class="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-btncolor focus:border-btncolor focus:ring-opacity-30" required>
              </div>
              <div>
      ...
      <div x-data="{ selectedGrant: '' }" id="grantModal-{{ $member->id }}">
        <!-- Grant Selection -->
        <fieldset class="grid grid-cols-1 border border-gray-300 p-4 mt-4 rounded-md">
          <legend class="text-2xl font-semibold text-customIT mb-2">Available Grants & Equipments</legend>

          <label class="block text-sm font-medium text-gray-700">Grants</label>
          <select name="grant_id"
                  x-model="selectedGrant"
        ...
        <!-- Requirements Checklist (auto shown based on selected grant) -->
        @foreach($grants as $grant)
          <fieldset class="grid grid-cols-1 border border-gray-300 p-4 mt-4 rounded-md"
              x-show="selectedGrant == '{{ $grant->id }}'"
              x-transition
              style="display: none;">
            <legend class="text-2xl font-semibold text-customIT mb-4">
              Requirements Checklist for {{ $grant->title }}
            </legend>
          
            @foreach($grant->grant_requirements as $grantReq)
              <div class="flex flex-col sm:flex-row sm:items-center justify-between border border-gray-200 rounded-lg p-3 mb-3 shadow-sm">
                <span class="text-sm font-medium text-gray-800 sm:w-1/2">
                  {{ $grantReq->requirement->requirement_name }}
                </span>
                
                <div class="mt-2 sm:mt-0 sm:w-1/2">
                  <input type="file" name="documents[{{ $grantReq->id }}]" class="w-full text-sm border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-btncolor focus:border-btncolor transition">
                </div>
              </div>
            @endforeach
          </fieldset>
